...
purchasesRouter.get('/:id', async (req, res, next) => {
  try {
    ...
    const out = await query(
      `SELECT p.id,p.status,p.amount_cents,p.currency,p.created_at,p.paid_at,b.title,b.slug,b.author
  ...
  try {
    ...
    const result = await tx(async client => {
      const p = await client.query(
        `SELECT p.*, b.title FROM purchases p JOIN books b ON b.id=p.book_id WHERE p.id=$1 AND p.access_token_hash=$2 FOR UPDATE`,
        [req.params.id, tokenHash]
      );
      ...
      if (existing.rowCount) return { status: 409 as const, error: 'An active one-time link already exists. Use it or wait for it to expire.' };
      const link = await createDownloadLink(client, p.rows[0].id, p.rows[0].book_id);
      return { status: 201 as const, token: link.token, expiresAt: link.expiresAt };
    });

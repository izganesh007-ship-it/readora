...
booksRouter.get('/', async (req, res, next) => {
  try {
    ...
    if (q.q) { params.push(q.q); where.push(`b.search_vector @@ websearch_to_tsquery('english', $${params.length})`); }
    if (q.category) { params.push(q.category); where.push(`c.slug = $${params.length}`); }
    if (q.access) { params.push(q.access); where.push(`b.access = $${params.length}`); }
    ...
    const rows = await query(
      `SELECT b.id,b.title,b.slug,b.author,b.description,b.access,b.price_cents,b.currency,b.cover_key,b.rating,b.review_count,b.popularity_score,c.name AS category,c.slug AS category_slug
...
booksRouter.get('/:slug', async (req, res, next) => {
  try {
    ...
    if (!row.rowCount) return res.status(404).json({ error: 'Book not found' });

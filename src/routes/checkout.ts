...
checkoutRouter.post('/btcpay', async (req, res, next) => {
  try {
    ...
    const purchase = await query(
      `INSERT INTO purchases(book_id,buyer_email,amount_cents,currency,status,access_token_hash) VALUES($1,$2,$3,$4,'PENDING',$5) RETURNING *`,
      [body.bookId, body.buyerEmail || null, book.rows[0].price_cents, book.rows[0].currency, hashPurchaseAccessToken(purchaseAccessToken)]
    );

...
if (-not (Test-Path (Join-Path $ProjectRoot "package.json"))) {
  throw "Cannot find package.json. Run this script inside the Readora project folder."
}
...
checkoutRouter.post('/btcpay', async (req, res, next) => {
  try {
    ...
    if (book.rows[0].access === 'FREE') return res.status(400).json({ error: 'Free books do not require checkout' });
    ...
    if (env.USE_NOWPAYMENTS) {
      const payment = await createNowPayment(
        purchaseId,
        book.rows[0].price_cents,
        (book.rows[0].currency || 'USD').toLowerCase(),
        `Readora ebook: ${book.rows[0].title}`
      );

      await query(
        'UPDATE purchases SET btcpay_invoice_id=$1, btcpay_checkout_link=$2 WHERE id=$3',
        [payment.paymentId, payment.paymentUrl, purchaseId]
      );
    ...
    await query(
      'UPDATE purchases SET btcpay_invoice_id=$1, btcpay_checkout_link=$2 WHERE id=$3',
      [invoice.id, invoice.checkoutLink, purchaseId]
    );
...
# -------------------------------
# 6. Update public/index.html script safely
# -------------------------------
...
<div class="app">
  ...
  <footer class="footer">
    <div><b>Readora</b> — premium ebook marketplace with automatic crypto checkout and one-time downloads.</div>
    <div>
...
<script>
(function () {
  console.log('[Readora] SPA loaded with NOWPayments support');
  ...
  async function startCheckout(bookId) {
    ...
    try {
      ...
      console.error('[Readora] checkout failed', err);
  ...
  async function pollPaymentStatus(pending) {
    console.log('[Readora] polling payment status', pending.purchaseId);
    ...
    try {
      ...
      console.error('[Readora] payment poll failed', err);
  ...
  async function unlockDownloadToken(pending) {
    ...
    try {
      ...
      console.error('[Readora] download token failed', err);
...
# -------------------------------
# 9. Optional env examples
# -------------------------------
$EnvNotePath = Join-Path $ProjectRoot ".env.nowpayments.example"
$EnvNote = @'
# Add these on Railway Variables to enable NOWPayments.
# Keep existing DATABASE_URL, COOKIE_SECRET, JWT_SECRET, APP_URL, CORS_ORIGIN, storage variables.
...
if ($Status) {
  git commit -m "Replace demo crypto checkout with NOWPayments integration"
} else {

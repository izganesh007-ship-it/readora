...
downloadsRouter.post('/:token/redeem', async (req, res, next) => {
    try {
        ...
        const result = await tx(async (client) => {
            const link = await client.query(`SELECT dl.*, b.title, b.epub_key, b.pdf_key FROM download_links dl
         JOIN books b ON b.id=dl.book_id
         WHERE dl.token_hash=$1 FOR UPDATE`, [tokenHash]);
            if (!link.rowCount)
                return { status: 404, error: 'Download link not found' };
            ...
            if (row.status !== 'ACTIVE' || new Date(row.expires_at) < new Date()) {
                ...
                return { status: 410, error: 'Download link expired' };
            }
            const key = row.epub_key || row.pdf_key;
            if (!key)
                return { status: 404, error: 'No ebook file is configured' };
            ...
            return { status: 200, url: await signedObjectUrl(key, `${row.title}.epub`, 'attachment') };
        });
        if ('error' in result)

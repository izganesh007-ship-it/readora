...
async function createPurchase(bookId, buyerEmail) {
    const book = await query('SELECT id,title,slug,access,price_cents,currency FROM books WHERE id=$1 AND is_active=true', [bookId]);
    if (!book.rowCount)
        return { errorStatus: 404, error: 'Book not found' };
    if (book.rows[0].access === 'FREE')
        return { errorStatus: 400, error: 'Free books do not require checkout' };

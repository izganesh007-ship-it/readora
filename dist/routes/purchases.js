...
purchasesRouter.get('/:id', async (req, res, next) => {
    try {
        ...
        const out = await query(`SELECT p.id,p.status,p.amount_cents,p.currency,p.created_at,p.paid_at,p.btcpay_checkout_link,b.title,b.slug,b.author
       FROM purchases p
...
purchasesRouter.post('/:id/download-token', async (req, res, next) => {
    try {
        ...
        const result = await tx(async (client) => {
            const p = await client.query(`SELECT p.*, b.title
         FROM purchases p
         JOIN books b ON b.id=p.book_id
         WHERE p.id=$1 AND p.access_token_hash=$2
         FOR UPDATE`, [req.params.id, tokenHash]);
            if (!p.rowCount)
                return { status: 404, error: 'Purchase not found' };
            if (p.rows[0].status !== 'PAID')
                return { status: 402, error: 'Payment not confirmed yet' };
            const used = await client.query(`SELECT id FROM download_links
         WHERE purchase_id=$1 AND status='USED'
         LIMIT 1`, [p.rows[0].id]);
            if (used.rowCount) {
                return { status: 410, error: 'Download already used' };
            }
            const active = await client.query(`SELECT token_hash, expires_at FROM download_links
         WHERE purchase_id=$1 AND status='ACTIVE' AND expires_at > now()
         LIMIT 1`, [p.rows[0].id]);
            if (active.rowCount) {
                await client.query(`UPDATE download_links
           SET status='REVOKED'
           WHERE purchase_id=$1 AND status='ACTIVE'`, [p.rows[0].id]);
            }
